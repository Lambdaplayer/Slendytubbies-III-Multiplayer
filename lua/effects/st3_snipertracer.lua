...
local tracer = Material("effects/beam_generic01")
local smoke = Material("effects/beam_generic01")
...
function EFFECT:Render()
    ...
    local col = LerpColor(d, self.Color, Color(255, 153, 0, 255))
    local col2 = LerpColor(d2, Color(255, 153, 0, 255), Color(255, 153, 0, 255))

    -- render.SetMaterial(head)
    -- render.DrawSprite(endpos, size * 3, size * 3, col)

    render.SetMaterial(tracer)
    render.DrawBeam(endpos, startpos, size, 7, 7, col)

    render.SetMaterial(smoke)
    render.DrawBeam(self.EndPos, self.StartPos, size * 1.9 * d2, 0, 1, col2)
end
